    ...
    public void execute() {
        ...
        for (int i = 0; i < code.length(); ) {
            switch (code.charAt(i)) {
                ...
                } code = codebackup.substring(procidx[operand[ptr]], itmp - 1); execute(); i = itmp; code = codebackup;
                    continue outer; case '<': if (ptr - 1 < 0) {ptr = size - 1;} else {--ptr;} break;
                case '>': if (ptr + 1 > size) {ptr = 0;} else {++ptr;} break; case '+': ++operand[ptr]; break;
                case '-': --operand[ptr]; break;
