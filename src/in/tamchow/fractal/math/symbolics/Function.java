...
public class Function {
    ...
    public void setConsts(String[][] constdec) {
        consts = new String[constdec.length][constdec[0].length]; for (int i = 0; i < this.consts.length; i++) {
    ...
    public void setSigns(ArrayList<String> signs) {
        this.signs.clear(); this.signs.addAll(signs);
    }
    public ArrayList<FunctionTerm> getTerms() {
        return terms;
    }
    public void setTerms(ArrayList<FunctionTerm> terms) {
        this.terms.clear(); this.terms.addAll(terms);
    }
    public String derivative(int order) {
        String deriv = ""; switch (order) {
            case 1: for (int i = 0, j = 0; i < terms.size() && j < signs.size(); i++, j++) {
                deriv += " " + signs.get(j) + " " + terms.get(i).derivative(1);
            } if (deriv.trim().charAt(0) == '+') {return deriv.trim().substring(1, deriv.trim().length());} break;
            case 2: for (int i = 0, j = 0; i < terms.size() && j < signs.size(); i++, j++) {
    ...
    public Complex getDegree() {
        Complex degree = new Complex(Complex.ZERO); for (FunctionTerm term : terms) {
            Complex vardeg = term.getDegree(); if (vardeg.modulus() > degree.modulus()) {
