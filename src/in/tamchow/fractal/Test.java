...
public class Test {
    public static void main(String[] args) {
        String func = "( z ^ 3 ) + ( ( d ) * ( z ) ) + e", variableCode = "z", poly = "{1:z:2};+;{e:z:0}", poly2 = "{f:z:0};sin;{1:z:1}", func2 = "z ^ 2 + e";
        String[][] consts = {{"c", "-0.1,+0.651i"}, {"d", "-0.7198,+0.9111i"}, {"e", "-0.8,+0.156i"}, {"f", "-0.5,+0.25i"}, {"g", "1,+0.3i"}};
        int resx = 640, resy = 480, fracmode = ComplexFractalGenerator.MODE_MANDELBROT, iter = 32, switch_rate = 3;
        double escrad = 10, tolerance = 1e-15, zoom = 10, zoompow = 0, baseprec = 100;
        ColorConfig cfg = new ColorConfig(Colors.CALCULATIONS.DISTANCE_ESTIMATION_COLOR, 4, 10, true, true);
        ...
        boolean def = (args.length == 0); ComplexFractalConfig fccfg = new ComplexFractalConfig(0, 0, 0); if (!def) {
            ...
            jgen = new ComplexFractalGenerator(resx, resy, zoom, zoompow, baseprec, fracmode, func, consts, variableCode, tolerance, cfg, new DesktopProgressPublisher(), switch_rate);
