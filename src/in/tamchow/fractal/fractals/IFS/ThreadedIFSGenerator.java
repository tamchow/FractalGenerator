...
public class ThreadedIFSGenerator extends ThreadedGenerator {
    ...
    public ThreadedIFSGenerator(IFSGenerator generator) {
        master = generator; threads = master.getParams().getThreads();
    }
    ...
    public int countCompletedThreads() {
        int ctr = 0; for (PartIFSData partImage : data) {if (partImage != null) ctr++;} return ctr;
    }
    @Override
    ...
    public void generate() {
        int idx = 0; for (int i = 0; i < threads; i++) {
            ...
            synchronized (lock) {
                while (!allComplete()) {lock.wait(1000);} lock.notifyAll(); for (PartIFSData partIFSData : data) {
