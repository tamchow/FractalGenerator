    ...
    public synchronized int getColor(int val, Complex[] last, double escape_radius, int iterations) {
        ...
        switch (color.getMode()) {
            ...
            case Colors.CALCULATIONS.DISTANCE_ESTIMATION_COLOR: double distance; if ((mode == MODE_BUDDHABROT || mode == MODE_MANDELBROT)) {
                ...
                index = color.createIndex((distance - Math.floor(distance)), 0, 1, scaling);
            ...
            case Colors.CALCULATIONS.COLOR_NEWTON_STRIPES:
                ...
                break;
            case Colors.CALCULATIONS.COLOR_NEWTON_NORMALIZED: color1 = color.getTint(color.getColor((indexOfRoot(last[0]) * color.color_density) % color.num_colors), ((double) val / iterations)); color2 = color.getTint(color.getColor((indexOfRoot(last[0]) * color.color_density) % color.num_colors), ((double) (val + 1) / iterations)); color3 = color.getTint(color.getColor((indexOfRoot(last[0]) * color.color_density) % color.num_colors), ((double) Math.abs(val - 1) / iterations)); colortmp1 = ColorConfig.linearInterpolated(color1, color2, val, iterations, color.isByParts()); colortmp2 = ColorConfig.linearInterpolated(color3, color1, val, iterations, color.isByParts()); colortmp = ColorConfig.linearInterpolated(colortmp2, colortmp1, val, iterations, color.isByParts()); break;
