...
public class FractalPanningHelper {
    public static synchronized ImageData pan(Pannable toPan, int distance, double angle) {return pan(toPan, distance, angle, false);}
    ...
    public static synchronized ImageData pan(Pannable toPanthis, int x_dist, int y_dist) {
        if (toPanthis instanceof ComplexFractalGenerator) {
            ComplexFractalGenerator toPan = (ComplexFractalGenerator) toPanthis; toPan.pan(x_dist, y_dist);
            int start_x, end_x, start_y, end_y; if (x_dist < 0) {
                start_x = toPan.getArgand().getWidth() + x_dist; end_x = toPan.getArgand().getWidth();
            } else {
                start_x = 0; end_x = x_dist;
            } start_y = 0; end_y = toPan.getArgand().getHeight();
            ThreadedComplexFractalGenerator panner = new ThreadedComplexFractalGenerator(toPan);
            panner.generate(start_x, end_x, start_y, end_y); if (y_dist < 0) {
            ...
            toPanthis.pan(x_dist, y_dist); return (ImageData) toPanthis;
        } else {
